...
{    
     ...
     */
    private DataFileLoaderFrame(DataSource dataSource)
    {
	super();
	setTitle("New data");
	chooser.setDialogType(JFileChooser.OPEN_DIALOG);
	chooser.setCurrentDirectory(new File("data in/csv/"));
	filter  = new ExampleFileFilter();
	filter.addExtension("csv");
	filter.setDescription("Comma Seperated Var files");
     ...
     *
     */
    public static DataFileLoaderFrame getInstance(DataSource dataSource)
    {
	if (instance != null)
	{
		instance.dispose();
	...
	try 
	{
	    ...
	    // Send the CSV data to the calling visual modulke (DataSource)
	    
	    dataSource.setCSVData(loader.getDataItemCollection());
	    dataSource = null;
