...
//import source.alg.PivotSpringModel;
								...
								java.io.Serializable
{
	...
	// The parent SpringModel module
        ...
        //private PivotSpringModel pivotSpringModel;
	...
	private int DESIRED_HEIGHT = 359;
    ...
    private boolean pivotSpring = false;
	
    public SpringModelControlPanel(SpringModel springModel)
    {
	this.springModel = springModel;
	construct();
    }
    
    /*public SpringModelControlPanel(PivotSpringModel springModel)
    {
	this.pivotSpringModel = springModel;
	pivotSpring = true;
	construct();
    }*/
	...
	*/
	...
	{	
		...
		/*if (pivotSpring)
		{
		    if (pivotSpringModel.isConvergenceTrigger())
			bConTrigger = true;
		    else
			bConTrigger = false;
		
		    if (pivotSpringModel.getConvergenceTrigger() == SpringModel.ITERATIVE_CONVERGENCE)
			{
				rbIterationTrigger.setSelected(true);
				txtTriggerVal.setDocument(new ValidateTriggerField(null));
				txtTriggerVal.setText((new Integer(pivotSpringModel.getIterationTrigValue())).toString());
				txtTriggerVal.getDocument().addDocumentListener(this);
			}
			else if (pivotSpringModel.getConvergenceTrigger() == SpringModel.VELOCITY_CONVERGENCE)
			{	
				rbVelocityTrigger.setSelected(true);
				txtTriggerVal.setDocument(new ValidateTriggerField(doubleFormat));
				txtTriggerVal.setText((new Double(pivotSpringModel.getVelocityTrigValue())).toString());
				txtTriggerVal.getDocument().addDocumentListener(this);
			}
			
			chkEnableConTrigger.setSelected(bConTrigger);
			enableDisableTriggerControls(pivotSpringModel.isTriggerEnabled());
		} else {*/
		     if (springModel.isConvergenceTrigger())
			bConTrigger = true;
		    else
			bConTrigger = false;
		     if (springModel.getConvergenceTrigger() == SpringModel.ITERATIVE_CONVERGENCE)
			 {
			     rbIterationTrigger.setSelected(true);
			     txtTriggerVal.setDocument(new ValidateTriggerField(null));
		     ...
		     else if (springModel.getConvergenceTrigger() == SpringModel.VELOCITY_CONVERGENCE)
			 {	
			     rbVelocityTrigger.setSelected(true);
			     txtTriggerVal.setDocument(new ValidateTriggerField(doubleFormat));
	...
	*/
	...
	public void actionPerformed(ActionEvent e)
	{
			...
			(e.getSource() == rbVelocityTrigger))
		{
			...
			/*if (pivotSpring){
			    if (rbIterationTrigger.isSelected())
				pivotSpringModel.setConvergenceTrigger(SpringModel.ITERATIVE_CONVERGENCE);
			    else
				pivotSpringModel.setConvergenceTrigger(SpringModel.VELOCITY_CONVERGENCE);
 // Set the text field's document
			    ...
			    if (rbIterationTrigger.isSelected())
				springModel.setConvergenceTrigger(SpringModel.ITERATIVE_CONVERGENCE);
			    else
				springModel.setConvergenceTrigger(SpringModel.VELOCITY_CONVERGENCE);
	...
	public void itemStateChanged(ItemEvent e)
	{
		if (e.getSource() == chkEnableConTrigger)
		{
			...
			if (e.getStateChange() == ItemEvent.SELECTED)
			{
				...
				pivotSpringModel.setConvergenceTriggerEnabled(true);
			...
			{
				pivotSpringModel.setConvergenceTriggerEnabled(false);
	...
	*/
	...
	private void updateSpringModelConTrigger()
	{
		...
		if (txtTriggerVal.getText().length() > 0)
		    {
			if (rbIterationTrigger.isSelected())
			    pivotSpringModel.setIterationTrigValue(Integer.parseInt(txtTriggerVal.getText()));
			else
			    pivotSpringModel.setVelocityTrigValue(Double.parseDouble(txtTriggerVal.getText()));
		    }
	    } else{*/
		if (txtTriggerVal.getText().length() > 0)
		    {
	...
	public void enableDisableTriggerControls(boolean bEnable)
	{
		...
		/*if(pivotSpring)
		    pivotSpringModel.setConvergenceTriggerEnabled(chkEnableConTrigger.isSelected());
		else*/
	...
	*/
		...
		if (!source.getValueIsAdjusting()) 
		{
			double val = (double)source.getValue() / (double)SLIDER_MAX;
	    		...
	    		/*if (pivotSpring){
			     if (source == freeness) 
				{
				    pivotSpringModel.setFreeness(val);
				}
			    else if (source == spring)
				{ 
				    pivotSpringModel.setSpringForce(val);
				}
			    else if (source == damping)
				{
				    pivotSpringModel.setDampingFactor(val);
				}
			}else{*/
			    if (source == freeness) 
				{
    	...
    	private void resizeModuleForShow()
	{
	    ...
	    /*if (pivotSpring)
		originalSize = pivotSpringModel.getSize(null);
	    else*/
		...
		{	
			...
			{	
				...
				/*if (pivotSpring){
				    pivotSpringModel.setDimension((new Double(currentWidth)).intValue(),
			...
			/*if (pivotSpring){
			    pivotSpringModel.validate();
			    pivotSpringModel.getParentForm().getDrawPane().repaint();
			}else {*/
			    springModel.validate();
		...
		/*if (pivotSpring){
		    pivotSpringModel.setSelected(true);
		    pivotSpringModel.repaint();
		}else{*/
    	...
    	private void resizeModuleForHide()
	{
	   ...
	   /* if (pivotSpring){
		currentWidth  = pivotSpringModel.getSize(null).getWidth();
		currentHeight  = pivotSpringModel.getSize(null).getHeight();
	    }else{*/
		currentWidth  = springModel.getSize(null).getWidth();
		currentHeight  = springModel.getSize(null).getHeight();
