 ...
 * RadialInterpolation
 *
 * Class represents an instance of a visual module for using radial, 
 * binary quadrant search interpolation
 *
...
public class RadialInterpolation extends DefaultVisualModule implements Runnable
{
	...
	private int vectorIterations  = 20;
	...
	public RadialInterpolation(Mdi mdiForm, DrawingCanvas drawPane)
	{
		super(mdiForm, drawPane);

		setName("Interpolation");
		setToolTipText("Radial binary search interpolation");
	...
	*/
	private void placeNearToNearestNeighbour(int index, int parent, ArrayList sample, double[] sampCache)
	{
		...
		for ( int i = 0 ; i < vectorIterations ; i++ ) 
		{
		    ...
		    Vect v = sumForcesToSample(index, sample, sampCache);
