...
public abstract class FileHandlerFrame extends JFrame implements ActionListener
{
    ...
    // Determine whether we're opening a directory for indexing
    
    protected boolean bOpenForIndexing = false;
    
    /**
     * constructor: 
     *
     */
    public FileHandlerFrame()
    {
	...
	if (bOpenForIndexing)
		setTitle("Index docs & open as data source");
	else
		setTitle("Open a data source");
	...
	if (e.getActionCommand().equals("ok"))
	{
		...
		if (bOpenForIndexing)
		{
			...
			Indexer indx = new Indexer(fileText.getText());
			indx = null;
			System.gc();
			
			// Open the data, which the index puts in a folder of the same name as the documents
			// but affixed with the word '_index'
			
			fileText.setText(fileText.getText() + "_index");
			handleFile();
		}
		else
			handleFile();
